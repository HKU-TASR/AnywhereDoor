...
class BackdoorValLoop(ValLoop):
    def __init__(self,
        ...
        self.map_evaluator = Evaluator([dict(type='VOCMetric', metric='mAP', eval_mode='11points')])
    ...
    def run(self) -> dict:
        ...
        for metric in self.metrics:
            if metric not in ['clean_mAP', 'known_asr', 'unknown_asr']:
            ...
            for attack_type, attack_mode in self.type_mode_combinations:
                ...
                elif metric == 'known_asr':
                    self._switch_evaluator(self.asr_evaluator)
                    known = True
                elif metric == 'unknown_asr':
                    self._switch_evaluator(self.asr_evaluator)
                    known = False
                ...
                for idx, data_batch in enumerate(self.dataloader):
                    if metric != 'clean_mAP':
                        data_batch = self._modify_sample(data_batch, attack_type, attack_mode, known=known)
    ...
    def _modify_sample(self, data_batch, attack_type, attack_mode, known):
        ...
        for idx in range(len(data_batch['inputs'])):
            ...
            if attack_mode == 'targeted':
                victim_class, victim_idx, target_class, target_idx = core.init_victim_target_class(
                                                                        'val', attack_type, \
                                                                        available_classes, self.all_classes, None,\
            ...
            feature, curse = core.get_feature_in_val_loop(attack_type, attack_mode, self.stage, \
                              victim_idx, target_idx, victim_class, target_class, \
                              self.curse_templates, cache, \
                              self.input_dim, known)
            ...
            ###     modify image
            #############################################################################
            data_batch['inputs'][idx] = core.get_modified_image('val', self.stage, self.data_root, self.device, data_batch, idx, \
                                                                victim_idx, target_idx, victim_class, target_class, \
