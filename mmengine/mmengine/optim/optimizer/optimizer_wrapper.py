...
class OptimWrapper(BaseOptimWrapper):
    ...
    def update_params(  # type: ignore
        ...
        if self.should_update():
            ...
            self._print_detector()
            self.zero_grad(**zero_kwargs)

    def _print_detector(self):
        if self._inner_count % 100 != 0:
            return

        ##############################
        # logger: MMLogger = MMLogger.get_current_instance()

        # max_value = torch.cat([param.data.view(-1) for param in self.optimizer.param_groups[0]['params'] if param.data is not None], dim=0).max().item()
        # mean_value = torch.cat([param.data.view(-1) for param in self.optimizer.param_groups[0]['params'] if param.data is not None], dim=0).mean().item()
        # min_value = torch.cat([param.data.view(-1) for param in self.optimizer.param_groups[0]['params'] if param.data is not None], dim=0).min().item()

        # log_str = "\n==================================================================================\n"
        # log_str += "Detector Param Values\n"
        # log_str += f"Max Value: {max_value}\n"
        # log_str += f"Mean Value: {mean_value}\n"
        # log_str += f"Min Value: {min_value}\n"

        # log_str += "-------------------------------------------------------------------------------------\n"

        # max_grad = torch.cat([param.grad.view(-1) for param in self.optimizer.param_groups[0]['params'] if param.grad is not None], dim=0).abs().max().item()
        # mean_grad = torch.cat([param.grad.view(-1) for param in self.optimizer.param_groups[0]['params'] if param.grad is not None], dim=0).abs().mean().item()
        # min_grad = torch.cat([param.grad.view(-1) for param in self.optimizer.param_groups[0]['params'] if param.grad is not None], dim=0).abs().min().item()

        # log_str += "Detector Grad\n"
        # log_str += f"Max Grad: {max_grad}\n"
        # log_str += f"Mean Grad: {mean_grad}\n"
        # log_str += f"Min Grad: {min_grad}\n"
        # log_str += "==================================================================================\n"
